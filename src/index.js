...
app.post("/createProduct", async (req, res) => {
  try {
    //Recebeu do cleinte o body
    const data = req.body;

    // separando em variaveis o corpo da requisição para enviar ao banco de dados através da query.
    ...
    // Retornando ao usuário em formato JSON o novo produto criado.
...
app.get("/listAllProducts/:producer", async (req, res) => {
  try {
    ...
    const allProducts = await pool.query("SELECT * FROM product WHERE product_producer=$1", [
      producer,
    ]);
